...
def send_message(request, receiver_id, item_id=None):
    ...
    # 1. 获取历史聊天记录 (按时间顺序)
    history_messages = PrivateMessage.objects.filter(
        (Q(sender=request.user) & Q(receiver=receiver)) |
        (Q(sender=receiver) & Q(receiver=request.user))
    ).order_by('created_at')

    # 2. 将对方发给我的未读消息标记为“已读”
    # 既然已经在这个页面看到了历史记录，就应该算作已读
    history_messages.filter(sender=receiver, receiver=request.user, is_read=False).update(is_read=True)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # 发送后为了体验更好，刷新当前页可以看到刚发的消息
            if item:
                return redirect('send_message_item', receiver_id=receiver.id, item_id=item.id)
            else:
    ...
    context = {
        ...
        'receiver': receiver,
        'item': item,
        'history_messages': history_messages,  # 传递给模板
    }
